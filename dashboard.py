...
sns.heatmap(filtered_df[['temp', 'hum', 'windspeed', 'cnt']].corr(), annot=True, cmap='coolwarm', fmt=".2f", linewidths=0.5, ax=ax)
st.pyplot(fig)

# Grafik Peminjaman Sepeda per Hari
...
sns.lineplot(data=weekday_avg, markers=True, dashes=False, ax=ax)
...
# Distribusi Suhu terhadap Peminjaman
st.subheader("Distribusi Suhu dan Pengaruhnya terhadap Peminjaman")
fig, ax = plt.subplots(figsize=(10, 6))
sns.scatterplot(data=filtered_df, x="temp", y="cnt", alpha=0.6, ax=ax)
ax.set_xlabel("Suhu")
